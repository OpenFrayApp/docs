#!/bin/bash
# Cut the site's vignettes from the shot heroes in out/, into the site clone's
# assets. Run after re-shooting console-hero and apply-effect-hero; each crop is
# a region of its source, so the vignettes always match the full screenshots
# they open into. Boxes are in source pixels (heroes are shot at 2x).
set -euo pipefail
cd "$(dirname "$0")"

OUT=out
SITE=../../site/src/assets/screenshots

# From console-hero.png (3600x2000): the tracker column, the log rail, and the
# lone Greatclub entry the home page narrates.
magick "$OUT/console-hero.png" -crop 880x1214+40+244 +repage "$SITE/console-rows.png"
magick "$OUT/console-hero.png" -crop 780x994+2800+872 +repage "$SITE/console-log.png"
magick "$OUT/console-hero.png" -crop 764x290+2798+930 +repage "$SITE/greatclub-log.png"

# From apply-effect-hero.png (1080x2058, already clipped to the modal): its four
# zones, in the order the page tells them.
magick "$OUT/apply-effect-hero.png" -crop 1020x270+30+515 +repage "$SITE/apply-conditions.png"
magick "$OUT/apply-effect-hero.png" -crop 990x300+45+1290 +repage "$SITE/apply-modifier.png"
magick "$OUT/apply-effect-hero.png" -crop 1020x250+30+1015 +repage "$SITE/apply-counter.png"
magick "$OUT/apply-effect-hero.png" -crop 1020x185+30+1700 +repage "$SITE/apply-name.png"

echo "vignettes recut into $SITE"
